...
public class GraphPanel extends JPanel implements ActionListener{
	...
	public GraphPanel(final AccountingGUI main) {
		...
		if(main.getDefaultGraphState().getTraceList() != null && main.getDefaultGraphState().getTraceList().size() > 0) {
			...
			main.updateStatus("Oletusuraa ei löydy");
		}
	...
	private JPanel createOptionsPanel() {
		...
		constraintsL.gridy++;
		constraintsR.gridy++;
		JLabel intervalLabel = new JLabel("Aikaväli");
		...
		constraintsR.gridx++;
		JLabel intervalSelectorLabel = new JLabel("Askelväli");
		...
		addTraceButton = new JButton("Lisää ura");
	...
	public GraphState collectGraphInformation() {
		...
		try {
			...
			//main.updateStatus("Alkupäivämäärä ei kelpaa");
		}
		try {
			...
			//main.updateStatus("Loppupäivämäärä ei kelpaa");
		}
		if(startDate != null && endDate != null) {
			if(startDate.after(endDate)) {
				...
				startDate = endDate;
				endDate = temp;
				main.updateStatus("Päivämäärät olivat väärinpäin. Virhe korjattu");
			}
		}
